...
const cartReducer = (state, action) => {
	switch (action.type) {
		...
		case "TOGGLE_PROTECTION": {
			const { payload: protectionCost } = action;
			return {
				...state,
				protectionCost,
			};
...
const CartProvider = ({ children }) => {
	...
	// Adding state to manage the shipping cost and protection cost
	const [state, dispatch] = useReducer(cartReducer, {
		cartItems: storedCartItems,
		totalPrice: storedTotalPrice,
		protectionCost: 0,
	...
	// Action to toggle protection cost
	const toggleProtection = (isChecked) => {
		dispatch({ type: "TOGGLE_PROTECTION", payload: isChecked ? 80.3 : 0 });
	};
