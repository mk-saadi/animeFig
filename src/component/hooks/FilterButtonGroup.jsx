...
const FilterButtonGroup = ({
	...
	// Reorder the filterValues array so that the selectedFilter comes first
	const reorderedFilterValues = selectedFilter
		? [selectedFilter, ...filterValues.filter((value) => value !== selectedFilter)]
		: filterValues;
	...
	return (
		<div className="relative flex flex-col pb-3 border-b border-ash/30">
			...
			<div className="max-h-[20rem] ml-4 justify-start focus:outline-none text-sm items-start overflow-y-auto flex flex-col gap-y-3">
				{reorderedFilterValues.map((value) => (
					<button
						key={value}
					...
					>
						{value} <span className="mx-2.5">-</span> {filterCounts[value]}
					</button>
				))}
				{selectedFilter && (
					<button
